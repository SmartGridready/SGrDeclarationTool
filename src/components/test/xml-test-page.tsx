...
interface XmlTestPageProps<T> {
  parse: (xmlString: string) => Promise<T>;
  build: (data: T) => Promise<string>;
}
...
export function XmlTestPage<T>({ parse, build }: XmlTestPageProps<T>) {
  ...
  const handleFileChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const parsed = await parse(fileContent);
      const rebuilt = await build(parsed);
