...
import { getFirstElement, mapOptionalArray, setOptionalField, Xml2JsObject } from "@/utils/mapper-utils";
...
export function mapModbusInterface(modbusInterfaceXml: Xml2JsObject | undefined): ModbusInterface {
  ...
  const modbusInterfaceDescriptionXml = getFirstElement(modbusInterfaceXml, "modbusInterfaceDescription");
  ...
  const modbusInterface: ModbusInterface = {
    ...
    functionalProfileList: mapModbusFunctionalProfileList(getFirstElement(modbusInterfaceXml, "functionalProfileList")),
  };
