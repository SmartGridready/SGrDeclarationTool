...
export function ConfigurationListBitmapForm({ configIndex, bitmap }: ConfigurationListBitmapFormProps) {
  ...
  const adaptedActions = useMemo<DataTypeProductBitmapSlice>(() => {
    return {
      setBitmapDataType: (bitmap) => configurationListActions.setConfigurationListBitmapDataType(configIndex, bitmap),
      addBitmapEntry: (entry) => configurationListActions.addConfigurationListBitmapEntry(configIndex, entry),
      ...
      updateBitmapEntryLiteral: (entryIndex, literal) =>
        configurationListActions.updateConfigurationListBitmapEntryLiteral(configIndex, entryIndex, literal),
      updateBitmapEntryHexMask: (entryIndex, hexMask) =>
        configurationListActions.updateConfigurationListBitmapEntryHexMask(configIndex, entryIndex, hexMask),
      updateBitmapEntryDescription: (entryIndex, description) =>
        configurationListActions.updateConfigurationListBitmapEntryDescription(configIndex, entryIndex, description),
      addEmptyBitmapEntry: () => configurationListActions.addEmptyConfigurationListBitmapEntry(configIndex),
    };
