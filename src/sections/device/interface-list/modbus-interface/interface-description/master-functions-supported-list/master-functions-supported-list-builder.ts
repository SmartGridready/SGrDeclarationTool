 ...
 */
export function buildMasterFunctionsSupportedList(list: MasterFunctionsSupportedList): Record<string, unknown> {
  // Validate using validation layer
  const validation = validateMasterFunctionsSupportedList(list);
  if (!validation.success) {
    const firstError = validation.errors?.issues[0];
    const errorMessage = firstError?.message || "Validation failed for master functions supported list";
